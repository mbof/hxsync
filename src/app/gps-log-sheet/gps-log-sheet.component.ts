...
import { LucideRoute, LucideDownload, LucideTrash2, LucideX } from '@lucide/angular';
...
export class GpsLogSheetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
      ...
      if (state === 'gpslog-view') {
        if (!this.shown) {
          ...
          setTimeout(() => {
            ...
            if (config.gpsLogData && config.gpsLogData.locus.sessions.length > 0) {
    ...
    this.configSubscription = this.deviceMgr.configSession.config.subscribe((config) => {
      if (this.shown && config.gpsLogData && !this.selectedSession && config.gpsLogData.locus.sessions.length > 0) {
        this.selectSession(config.gpsLogData.locus.sessions[0]);
      }
    });
  }
  ...
  private updateTrackOnMap() {
    ...
    const latLngs = this.selectedSession.waypoints.map((wp) => [wp.lat, wp.lon] as L.LatLngExpression);
    this.trackLine = L.polyline(latLngs, { color: '#007bff', weight: 4 }).addTo(this.map);
  ...
  formatTime(utc: number): string {
    const timezone = this.deviceMgr.configSession.config.getValue().gpsLogData?.timezone;
    const date = new Date(utc * 1000);
    
    if (timezone) {
      // Manual adjustment for local time display
      const localDate = new Date(date.getTime() + timezone.offsetMinutes * 60000);
  ...
  downloadAll() {
    const locus = this.deviceMgr.configSession.config.getValue().gpsLogData?.locus;
    if (!locus) return;
  ...
  async clearLogs() {
    if (confirm('Are you sure you want to delete all GPS logs from the device? This cannot be undone.')) {
