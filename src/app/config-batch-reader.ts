...
export type BatchReaderRanges = Map<MemoryRangeId, { start: number, end: number }>;
...
export class ConfigBatchReader {
  ...
  ranges: BatchReaderRanges = new Map<MemoryRangeId, { start: number, end: number }>();
  ...
  async read(progressCallback: (offset: number) => void): Promise<BatchReaderResults> {
    const results = new Map<MemoryRangeId, Uint8Array>();
    let previousOffsets = 0;
    let totalSize = 0;
    this.ranges.forEach((range) => { totalSize += range.end - range.start });
    for (const [id, range] of this.ranges.entries()) {
      results.set(id,
        await this.configProtocol.readConfigMemory(range.start, range.end - range.start, 
          (offset: number) => {
